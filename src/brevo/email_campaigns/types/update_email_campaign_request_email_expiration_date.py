# This file was auto-generated by Fern from our API Definition.

import typing

import pydantic
from ...core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from .update_email_campaign_request_email_expiration_date_unit import UpdateEmailCampaignRequestEmailExpirationDateUnit


class UpdateEmailCampaignRequestEmailExpirationDate(UniversalBaseModel):
    """
    To reduce your carbon footprint, set an expiration date for your email. If supported, it will be automatically deleted from the recipient’s inbox, saving storage space and energy.
    """

    duration: typing.Optional[int] = pydantic.Field(default=None)
    """
    Duration of the email expiry. maximum duration can be 3600 days or 480 weeks or 120 months.
    """

    unit: typing.Optional[UpdateEmailCampaignRequestEmailExpirationDateUnit] = pydantic.Field(default=None)
    """
    unit of the duration
    """

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
